...
[row,col]=find(abs(V)>500);     %%find 查找非零元素的索引和值  ---->为什么要大于50？答：绝对值>50就超出了边界
...
%%
hori=find(round(abs(p(:,1)),-4)==500 & p(:,2)<500 & p(:,2)>=-500);  
...
%%
vert=find(round(abs(p(:,2)),-4)==500 & p(:,1)<500 & p(:,1)>=-500);
...
Bound=find(round(abs(p(:,2)),-4)==500 | round(abs(p(:,1)),-4)==500);
...
hold off
%%数据处理  
%%依照方框大小而定，单元间的距离不会超过方框范围，但剖分会出现inf值，需取成+-500；
for i=1:length(V)
    if V(i,1) > 500 && V(i,1) ~= Inf
       V(i,1) = 500;
    end
    if V(i,1) < -500 && V(i,1) ~= Inf
       V(i,1) = -500;
    end
    if V(i,2) > 500 && V(i,2) ~= Inf
       V(i,2) = 500;
    end
    if V(i,2) < -500 && V(i,2) ~= Inf
       V(i,2) = -500;
